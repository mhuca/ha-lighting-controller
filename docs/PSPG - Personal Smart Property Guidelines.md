...
**Description:** This document serves as the "System Constitution" for the Lean Logic OS. It defines the mandatory hardware standards, logical intersections, and operational procedures required to maintain a predictable, high-reliability smart home environment.

## **📝 System Purpose**

The PSPG is designed to solve the "Automated Conflict" problem. It ensures that every area follows a predictable pattern (Enclosure vs. Transient) so that residents never have to "fight" the house logic.
...
**Situation:** Want lights on as you open the door, not after camera detection.

**Action:** Apply the entrance label to the door's binary sensor.

**Effect:** Grants 10s buffer for camera/logic latency.
